...
export default function MapWrapper({
  ...
  React.useEffect(() => {
    if (!showMap || typeof window === 'undefined' || !mapContainerRef.current)
      return
  ...
  function updateCenterLabels(L, mapInstance, items, getGeoJSON) {
    ...
    items.forEach((item) => {
      ...
      if (
        ...
        const adjustedLatLng = mapInstance.layerPointToLatLng([
          point.x,
          point.y,
        ])
  ...
  React.useEffect(() => {
    if (!showMap || !mapContainerRef.current || typeof window === 'undefined')
      return
    import('leaflet').then((L) => {
      if (!mapContainerRef.current) return
      if (!mapInstanceRef.current) {
          ...
          const geoLayer = L.geoJSON(displayGeo, {
            style: () => {
              const color = colorMode
                ? colorMap.get(
                    chipColorStrategy ? chipColorStrategy(item) : 'default'
                  ) || '#e5e7eb'
                : getColorFromId(id)
              return {
                color,
      ...
      if (autoFit) {
        ...
        if (signature !== previousItemsSignatureRef.current) {
          ...
          markersRef.current.forEach(
            (m) => m.getLatLng && bounds.extend(m.getLatLng())
          )
      ...
      >
        <div style={{ lineHeight: 1.3 }}>
          ...
          {cursorCoords
            ...
            : '-'}
          <br />
          <span style={{ fontWeight: 500 }}>BBox:</span>
          <br />
