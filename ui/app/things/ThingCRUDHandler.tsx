...
export const useThingCRUDHandler = ({
  ...
  const buildCreatePayload = (raw: any) => {
    ...
    if (Array.isArray(raw.Locations) && raw.Locations.length > 0) {
      ...
      raw.newLocations &&
      Array.isArray(raw.newLocations) &&
      raw.newLocations.length > 0
    ) {
      ...
      for (const d of payload.Datastreams) {
        ...
        if (!d['@iot.id']) {
          ...
          if (!d.ObservedProperty)
            return 'New Datastream missing ObservedProperty'
  ...
  const handleSaveEdit = async (updatedThing: any, originalThing: any) => {
    ...
    try {
      ...
      const payload: any = {
        ...
        properties:
          Array.isArray(updatedThing.properties) &&
          updatedThing.properties.length > 0
            ? Object.fromEntries(
      ...
      await updateData(
        `${item.root}(${originalThing['@iot.id']})`,
        token,
        payload
      )
      const data = await fetchData(item.root, token)
  ...
  const fetchThingWithExpand = async (thingId: string | number) => {
    const nested =
      siteConfig.items.find((i) => i.label === 'Things')?.nested || []
